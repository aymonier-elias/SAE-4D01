...
<section class="content formulaire-escape">
    ...
    <form method="post" action="index.php?action=modifierEscape" class="form-escape" enctype="multipart/form-data">
        ...
        <label>Latitude <input type="text" name="latitude" value="<?= htmlspecialchars($escape['Latitude'] ?? '') ?>" placeholder="ex: 48.58 (Nord-Sud)"></label>
        <label>Longitude <input type="text" name="longitude" value="<?= htmlspecialchars($escape['Longitude'] ?? '') ?>" placeholder="ex: 7.75 (Est-Ouest)"></label>
